...
export function parseUnionDef(
  ...
  if (
    ...
    const types = options.reduce((types: JsonSchema7Primitive[], x) => {
      const typeKey = getDefTypeName(x._def);
      const type = typeKey
        ? primitiveMappings[typeKey as ZodPrimitive]
        : undefined;
    ...
    const types = options.reduce(
      (acc: JsonSchema7Primitive[], x: { _def: any }) => {
        ...
        const value = (x._def as any).values
          ? (x._def as any).values[0]
          : (x._def as any).value;
    ...
    return {
      type: "string",
      enum: options.reduce((acc: string[], x) => {
        // In Zod V4, use entries instead of values
        const values = (x._def as any).entries
          ? Object.values((x._def as any).entries)
          : (x._def as any).values;
        return [...acc, ...values.filter((x: string) => !acc.includes(x))];
      }, []),
    };
  }
